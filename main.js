...
var backgrounds = document .querySelectorA11('.background');
...
if (imageIndex -1 >= 0) {
    ...
    // Add the 'next' class to the imoge after the current one
...
images.forEach((image, index) => {
    if(index !== imageindex && index !== (imageIndex - 1 + images.length) %
    images.Length && index !== (imageIndex + 1) % images.length) {
        image.classList.add('inactive');
    }
});
// Set the apacity of all the background divs to 0
    backgrounds.forEach((background) => {
        background.style.opacity = 0;
    });
// If the current image is active, set the opacity of the corresponding buckgrand div to 1
 if(images[imageIndex].cLassList.contans('active')) {
